...
void EventLoop::loop() {
    ...
    while (!quit_) {
         ...
         * 就会唤醒一个subLoop
         * mainLoop
         * 事先注册一个回调cb（需要subloop来执行），但是此subloop还在被poller_->poll语句阻塞
         * 通过eventfd wakeup
         * subloop后，执行下面的方法，执行之前mainloop注册的cb操作，有可能是一个cb也有可能是多个cb
         *
 ...
 * 在当前loop中执行cb
 *
 * 在讲解消息的发送过程时候，我们讲到为了保证对 buffer 和 socket 的写动作是在 IO
 线程中进行
